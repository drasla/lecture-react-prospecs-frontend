...
import type { Inquiry, InquiryStatus, InquiryType } from "../../types/inquiry.ts";
import { getMyInquiries } from "../../api/inquiry.api.ts";
...
function MyInquiryList() {
    ...
    const getTypeLabel = (type: InquiryType) => {
        // if
        // if (type === "DELIVERY") { return "배송" } else if (type === "PRODUCT") { return "상품" }
        // switch

        // Record 타입 : 이 변수는 객체가 저장이 될건데, 그 객체의 key는 Inquiry 타입이고, value는 string 타입
        ...
        if (status === "ANSWERED") {
            return (
                <span
                    className={twMerge(
                        ["px-2", "py-1"],
                        ["text-xs", "font-bold", "text-white", "bg-black", "rounded-sm"],
                    )}>
                    답변완료
                </span>
            );
            ...
            return (
                <span
                    className={twMerge(
                        ["px-2", "py-1"],
                        ["text-xs", "font-bold", "text-gray-500", "bg-gray-200", "rounded-sm"],
                    )}>
                    답변대기
                </span>
            );
    ...
    if (loading)
        return (
            <div className={twMerge(["text-center", "text-gray-500"])}>
                문의 내역을 불러오는 중...
            </div>
        );
    ...
    return (
        <div className="w-full mx-auto px-4">
            ...
            <div className="flex justify-between items-end mb-8 border-b-2 border-black pb-4">
                ...
                <button
                    onClick={() => navigate("/my/inquiries/write")}
            ...
            {inquiries.length === 0 ? (
                ...
                <div className={twMerge(["border-t", "border-gray-200"])}>
